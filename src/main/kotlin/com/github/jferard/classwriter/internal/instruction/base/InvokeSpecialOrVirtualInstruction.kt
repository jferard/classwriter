 ...
 */
class InvokeSpecialOrVirtualInstruction(private val opcode: Int,
                                        private val methodRef: MethodRef) :
        BaseInstruction {
    override fun preprocess(context: GlobalContext,
                            codeContext: MethodContext) {
        codeContext.offsetDelta(3)
        codeContext.assertTypeAssignable(methodRef.classVerificationType,
                codeContext.stackPop())
        for (type in methodRef.argTypes) {
            codeContext.assertTypeAssignable(type.verificationType, codeContext.stackPop())
        }
        val retType = methodRef.retType
        if (retType != null) {
    ...
    override fun encode(context: GlobalContext,
        ...
        val index: Int = context.addToPool(MethodRefEntry(methodRef))
        return EncodedIndexedInstruction(opcode, index)
    }
