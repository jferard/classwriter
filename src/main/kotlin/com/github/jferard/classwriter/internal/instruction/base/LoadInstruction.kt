 ...
 * Stack: () -> (objectref).
 */
class LoadInstruction(private val opcode: Int, private val index: Int) : BaseInstruction,
        EncodedInstruction {
    override fun preprocess(context: GlobalContext,
                            codeContext: MethodContext) {
        codeContext.offsetDelta(2)
        val type: VerificationType = codeContext.localsGet(index)
    ...
    override fun write(encodedWriter: InstructionEncodedWriter) {
        return encodedWriter.loadInstruction(opcode, index)
    }
