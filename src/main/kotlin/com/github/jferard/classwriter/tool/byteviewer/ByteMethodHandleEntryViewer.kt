...
import com.github.jferard.classwriter.pool.MethodHandleKind.toString
...
class ByteMethodHandleEntryViewer(private val kind: Int, private val referenceIndex: Int) :
        ConstantPoolEncodedEntryViewer {
    override fun view(
            entries: List<ConstantPoolEncodedEntryViewer>,
            i: Int): String {
        return kotlin.String.format(
                ByteEntryViewerFactory.NUM_FORMAT + JavapEntryViewerFactory.CODE_FORMAT +
                        "%-16s// %s", "#$i", "MethodHandle",
                "#$kind:#$referenceIndex", viewSummary(entries))
    }

    override fun viewSummary(
            entries: List<ConstantPoolEncodedEntryViewer>): String {
        return entries[referenceIndex-1].viewSummary(entries) + " <" +
                toString(kind) + ">"
    }
