...
class JavapNumberEntryViewer<T : Number>(private val code: String, private val number: T) :
        ConstantPoolEncodedEntryViewer {
    override fun view(
            entries: List<ConstantPoolEncodedEntryViewer>,
            i: Int): String {
        return kotlin.String.format(
                JavapEntryViewerFactory.NUM_FORMAT + JavapEntryViewerFactory.CODE_FORMAT + "%-16s// %s",
                "#$i", code, number, viewSummary(entries))
    }
