...
class InvokeDynamicEntryViewer(private val bootstrapMethodAttrIndex: Int,
                               private val nameAndTypeIndex: Int) :
        ConstantPoolEncodedEntryViewer {
    override fun view(
            entries: List<ConstantPoolEncodedEntryViewer>,
            i: Int): String {
        return kotlin.String.format(JavapEntryViewerFactory.CODE_FORMAT + "%-16s// %s",
                "#$i",
                "InvokeDynamic",
    ...
    override fun viewSummary(
            entries: List<ConstantPoolEncodedEntryViewer>): String { // needs the 4.7.23. The BootstrapMethods Attribute
        return "" + entries!![nameAndTypeIndex]!!.viewSummary(entries)
    }
