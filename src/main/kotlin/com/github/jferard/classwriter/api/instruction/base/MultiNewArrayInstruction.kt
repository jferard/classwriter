...
import com.github.jferard.classwriter.encoded.instruction.EncodedMultiNewArrayInstruction
 ...
 */
class MultiNewArrayInstruction(private val classRef: PlainClassRef, private val dimensions: Int) :
        BaseInstruction {
    override fun preprocess(context: GlobalContext,
                            codeContext: MethodContext) {
        codeContext.offsetDelta(4)
        var type: ValueType = classRef.toValueType()
        for (i in 0 until dimensions) {
            codeContext.assertTypeAssignable(VerificationType.INTEGER,
                    codeContext.stackPop())
            type = ValueType.array(type)
        }
        codeContext.stackPush(type.verificationType)
    }
    ...
    override fun encode(context: GlobalContext,
        ...
        return EncodedMultiNewArrayInstruction(
                index, dimensions)
    }
}