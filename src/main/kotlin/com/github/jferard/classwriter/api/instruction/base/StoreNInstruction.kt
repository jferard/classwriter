...
class StoreNInstruction(private val opcode: Int, private val localIndex: Int,
                        private val verificationType: VerificationType) : BaseInstruction,
        EncodedInstruction {
    override fun preprocess(context: GlobalContext,
                            codeContext: MethodContext) {
        codeContext.offsetDelta(1)
        val stackType: VerificationType = codeContext.stackPop()
        codeContext.assertTypeAssignable(verificationType, stackType)
        codeContext.localsSet(localIndex, stackType)
    }
    ...
    override fun write(encodedWriter: InstructionEncodedWriter) {
        return encodedWriter.storeNInstruction(opcode, localIndex, verificationType)
    }
