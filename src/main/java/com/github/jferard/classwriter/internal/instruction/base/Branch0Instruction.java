...
/**
 * if____: if ____ 0, branch to instruction at branchoffset (signed short constructed from
 * unsigned bytes branchbyte1 << 8 + branchbyte2).
 * Other bytes (count: 2): branchbyte1, branchbyte2.
 * Stack: (value) -> ().
 */
public class Branch0Instruction implements BaseInstruction {
    private final int opcode;
    private final Label label;

    public Branch0Instruction(int opcode, Label label) {
        this.opcode = opcode;
        this.label = label;
    }
    ...
    public void preprocess(GlobalContext context, MethodContext codeContext) {
        ...
        codeContext.storeBranch(this.label);
    }

    @Override
    public EncodedInstruction encode(GlobalContext context, MethodContext codeContext) {
        int branchIndex = codeContext.getLabelOffset(this.label);
        return new EncodedIndexedInstruction(this.opcode, branchIndex);
    }
}
