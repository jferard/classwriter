...
public class JsrItem implements PositionedItem {
    private final Instruction instruction;
    private int offset;

    public JsrItem(int offset, Instruction instruction) {
        this.offset = offset;
    ...
    public int updateShift(int curShift, TargetItem targetItem) {
        assert targetItem.getOffset() > BytecodeHelper.SHORT_MAX;
        if (targetItem.isItemForTarget(this.instruction)) {
            return curShift + 2;
        } else {
            return curShift;
        }
