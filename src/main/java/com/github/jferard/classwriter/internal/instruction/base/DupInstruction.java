 ...
 */
public class DupInstruction implements BaseInstruction, EncodedInstruction {
    @Override
    public void preprocess(GlobalContext context, MethodContext codeContext) {
        codeContext.offsetDelta(1);
        VerificationType valueType = codeContext.stackPeek();
    ...
    public Writable<?> toWritable(InstructionWritableFactory<?> writableFactory) {
        return writableFactory.dupInstruction();
    }
