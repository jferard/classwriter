 ...
 * </pre>
 */
public class WideIincInstruction implements BaseInstruction, EncodedInstruction {
    private final int index;
    private final int c;

    public WideIincInstruction(int index, int c) {
        this.index = index;
        this.c = c;
    }
    ...
    public void preprocess(GlobalContext context, MethodContext codeContext) {
        ...
        final VerificationType valueType = codeContext.localsGet(this.index);
    ...
    public Writable<?> toWritable(InstructionWritableFactory<?> writableFactory) {
        return writableFactory.wideIincInstruction(this.index, this.c);
    }

    @Override
    public int getSize() {
        return 2 * BytecodeHelper.BYTE_SIZE + 2 * BytecodeHelper.SHORT_SIZE;
    }
}
