...
/**
 * if____: if ____ value, branch to instruction at branchoffset (signed
 * short constructed from unsigned bytes branchbyte1 << 8 + branchbyte2).
 * Other bytes (count: 2): branchbyte1, branchbyte2.
 * Stack: (value1, value2) -> ().
 */

public class BranchInstruction implements BaseInstruction {
    private final int opcode;
    private final Label label;

    public BranchInstruction(int opcode, Label label) {
        this.opcode = opcode;
        this.label = label;
    }
