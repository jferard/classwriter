 ...
 * Stack: () -> (objectref).
 */
public class LoadInstruction implements BaseInstruction, EncodedInstruction {
    private final int opcode;
    private final int index;

    public LoadInstruction(int opcode, int index) {
        this.opcode = opcode;
        this.index = index;
    }
    ...
    public void preprocess(GlobalContext context, MethodContext codeContext) {
        ...
        VerificationType type = codeContext.localsGet(this.index);
        codeContext.stackPush(type);
    }
    ...
    public Writable<?> toWritable(InstructionWritableFactory<?> writableFactory) {
        return writableFactory.loadInstruction(opcode, index);
    }

    @Override
    public int getSize() {
        return 2 * BytecodeHelper.BYTE_SIZE;
    }
}
