 ...
 * For a long.
 */
public class ConstantStringValueAttribute implements FieldAttribute {
    private final String value;

    ConstantStringValueAttribute(String value) {
        this.value = value;
    }

    @Override
    public EncodedFieldAttribute encode(GlobalContext pool, MethodContext codeContext) {
        int attributeNameIndex = pool
                .addEncodedToPool(new Utf8Entry(ConstantValueAttributeFactory.CONSTANT_VALUE_NAME));
        int valueIndex = pool.addToPool(new StringEntry(this.value));
