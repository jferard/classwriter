...
public class NewArrayInstruction implements BaseInstruction {
    private PrimitiveValueType type;

    public NewArrayInstruction(PrimitiveValueType type) {
        this.type = type;
    }
    ...
    public void preprocess(GlobalContext context, MethodContext codeContext) {
        ...
        codeContext.stackPush(VerificationType.arrayFromPrimitiveType(this.type));
    }

    @Override
    public EncodedInstruction encode(GlobalContext context, MethodContext codeContext) {
        return new EncodedNewArrayInstruction(this.type.getAType());
    }
}
