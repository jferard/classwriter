 ...
 * <pre>{@code
 * same_frame_extended {
 *     u1 frame_type = SAME_FRAME_EXTENDED_BASE; /* 251 * /
 *     u2 offset_delta;
 * }
 * }<pre>
 */

public class EncodedSameFrameExtended implements EncodedStackMapFrame {
    private int offsetDelta;

    public EncodedSameFrameExtended(int offsetDelta) {
        this.offsetDelta = offsetDelta;
    }

    @Override
    public Writable<?> toWritable(StackMapFrameWritableFactory<?> writableFactory) {
        return writableFactory.sameFrameExtended(this.offsetDelta);
    }
