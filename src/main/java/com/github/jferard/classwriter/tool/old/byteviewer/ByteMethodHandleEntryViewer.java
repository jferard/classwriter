...
public class ByteMethodHandleEntryViewer implements ConstantPoolEncodedEntryViewer {
    private final int kind;
    private final int referenceIndex;

    public ByteMethodHandleEntryViewer(int kind, int referenceIndex) {
        this.kind = kind;
    ...
    public String view(List<ConstantPoolEncodedEntryViewer> entries, int i) {
        return String
                .format(ByteEntryViewerFactory.NUM_FORMAT + JavapEntryViewerFactory.CODE_FORMAT +
                                "%-16s// %s", "#" + i, "MethodHandle",
                        "#" + this.kind + ":#" + this.referenceIndex, this.viewSummary(entries));
    }
