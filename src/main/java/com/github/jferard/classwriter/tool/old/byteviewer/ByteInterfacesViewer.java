...
public class ByteInterfacesViewer implements InterfacesViewer {
    private List<InterfaceViewer> interfaceViewers;

    public ByteInterfacesViewer(List<InterfaceViewer> interfaceViewers) {
        this.interfaceViewers = interfaceViewers;
    }

    @Override
    public void view(Writer w) throws IOException {
        int count = this.interfaceViewers.size();
        w.append(String.format("%s, %s, // interfaces count: %d\n", ByteViewerFactory.hex(count >> 8),
                ByteViewerFactory.hex(count), count));
        for (InterfaceViewer interfaceViewer : this.interfaceViewers) {
