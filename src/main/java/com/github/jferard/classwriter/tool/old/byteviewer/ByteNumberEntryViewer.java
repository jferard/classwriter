...
public class ByteNumberEntryViewer<T extends Number> implements ConstantPoolEncodedEntryViewer {
    private final String code;
    private final T number;

    public ByteNumberEntryViewer(String code, T number) {
        this.code = code;
        this.number = number;
    }
    ...
    public String view(List<ConstantPoolEncodedEntryViewer> entries, int i) {
        return String
                .format(ByteEntryViewerFactory.NUM_FORMAT + JavapEntryViewerFactory.CODE_FORMAT +
                        "%-16s// %s", "#" + i, this.code, this.number, this.viewSummary(entries));
    }
