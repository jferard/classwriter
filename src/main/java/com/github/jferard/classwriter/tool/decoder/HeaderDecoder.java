...
public class HeaderDecoder implements Decoder<ClassWritableFactory<?>> {
    @Override
    public Header decode(DataInput input) throws IOException {
        int magic = input.readInt();
        if (magic != EncodedClassFile.MAGIC_NUMBER) {
            throw new IllegalStateException();
        }
        int minorVersion = input.readShort();
        int majorVersion = input.readShort();

        return new Header(minorVersion, majorVersion);
    }
}
