...
class ByteNameAndTypeEntryViewer implements ConstantPoolEncodedEntryViewer {
    private final int nameIndex;
    private final int descriptorIndex;

    public ByteNameAndTypeEntryViewer(int nameIndex, int descriptorIndex) {
    ...
    public String view(List<ConstantPoolEncodedEntryViewer> entries, int i) {
        return String
                .format(ByteEntryViewerFactory.NUM_FORMAT + "%s, %s, %s, %s, %s, // %s", "#" + i,
                        "ConstantTags.NAMEANDTYPE", ByteViewerFactory.hex(this.nameIndex >> 8),
                        ByteViewerFactory.hex(this.nameIndex),
    ...
    public String viewSummary(List<ConstantPoolEncodedEntryViewer> entries) {
        return entries.get(this.nameIndex).viewSummary(entries) + ":" +
                entries.get(this.descriptorIndex).viewSummary(entries).replace('/', '.');
    }
}
