...
public class FieldRefEntry implements ConstantPoolEntry {
    ...
    public FieldRefEntry(FieldOrMethodRef fieldOrMethodRef) {
        this.fieldOrMethodRef = fieldOrMethodRef;
    }

    @Override
    public int addToPool(GlobalContext pool) {
        EncodedFieldRefEntry fieldOrMethodRefInfo = this.encode(pool, MethodContext.create(0));
    ...
    @Override
    public EncodedFieldRefEntry encode(GlobalContext pool, MethodContext codeContext) {
        ClassEntry constantClass = new ClassEntry(this.fieldOrMethodRef.getClassRef());
        int classIndex = pool.addToPool(constantClass);
        NameAndTypeEntry constantNameAndType = new NameAndTypeEntry(
                this.fieldOrMethodRef.getName(), this.fieldOrMethodRef.toDescriptor());
        int nameAndTypeIndex = pool.addToPool(constantNameAndType);
        return new EncodedFieldRefEntry(classIndex, nameAndTypeIndex);
    }

    @Override
    public boolean equals(Object o) {
        if (o == this) return true;
        if (!(o instanceof FieldRefEntry)) return false;

        FieldRefEntry other = (FieldRefEntry) o;
